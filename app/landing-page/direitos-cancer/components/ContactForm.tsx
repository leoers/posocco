...
export default function ContactForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const text = `Olá! Meu nome é ${formData.nome}.%0A%0APossui laudo médico atualizado? ${formData.pergunta1}%0A%0ANecessidade: ${formData.pergunta2}`;
      ...
      <div className="container" style={{ ...styles.grid, gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(320px, 1fr))', gap: isMobile ? '40px' : '80px' }}>
        ...
        <div style={styles.formContainer} data-aos="fade-left" data-aos-delay="200">
          <form onSubmit={handleSubmit}>
            ...
            <div style={styles.formGroup}>
              <label style={styles.label}>Você possui laudo médico atualizado?</label>
            ...
            <div style={styles.formGroup}>
              <label style={styles.label}>Qual é a sua principal necessidade hoje (Tratamento, Previdência ou Isenções)?</label>
              <textarea required name="pergunta2" value={formData.pergunta2} onChange={handleChange} style={{ ...styles.input, height: '80px', resize: 'none' }} placeholder="Descreva sua necessidade..."></textarea>
            </div>
...
const styles = {
  section: {
    ...
    backgroundImage: `linear-gradient(to right, rgba(4, 27, 59, 0.85), rgba(4, 27, 59, 0.6)), url('/direitos-cancer/direitos-cancer_background-formulario.png')`,
