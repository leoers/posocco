...
export default function ContactForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const text = `Olá! Meu nome é ${formData.nome}.%0A%0APossui laudo médico atualizado? ${formData.pergunta1}%0A%0AMaior desafio: ${formData.pergunta2}`;
    ...
    <section id="formulario" style={{ ...styles.section, paddingTop: isMobile ? '60px' : '120px' }}>
      <div className="container" style={{ ...styles.grid, gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(320px, 1fr))', gap: isMobile ? '40px' : '80px' }}>
        ...
        <div data-aos="fade-right" style={{ textAlign: isMobile ? 'center' : 'left' }}>
          <h2 className="h2" style={{ ...styles.title, fontSize: isMobile ? '32px' : '40px' }}>
            Quer contar o que aconteceu com você?
          </h2>
          <p style={styles.subtitle}>
            Nossa equipe pode fazer uma avaliação técnica do seu caso para entender se você tem direito à compensação e como podemos te ajudar da forma mais rápida.
          </p>
        </div>
        ...
        <div style={styles.formContainer} data-aos="fade-left" data-aos-delay="200">
          <form onSubmit={handleSubmit}>
            ...
            <div style={styles.formGroup}>
              <label style={styles.label}>Você já possui laudo médico atualizado?</label>
            ...
            <div style={styles.formGroup}>
              <label style={styles.label}>Qual é o seu maior desafio hoje (Plano de saúde, Educação ou BPC)?</label>
              <textarea required name="pergunta2" value={formData.pergunta2} onChange={handleChange} style={{ ...styles.input, height: '80px', resize: 'none' }} placeholder="Descreva aqui o seu maior desafio..."></textarea>
            </div>
...
const styles = {
  section: {
    ...
    backgroundImage: `linear-gradient(to right, rgba(4, 27, 59, 0.85), rgba(4, 27, 59, 0.6)), url('/diretos-autista/direitos-autista_background-formulario.png')`,
